...
class Mediator(handlers: Map[Class[_ <: Request[_]], RequestHandler[Request[_], _]]) extends MediatorDuty {
  ...
  override def send[TResponse](request: Request[TResponse]): Future[TResponse] = {
    ...
    handler.asInstanceOf[RequestHandler[Request[TResponse], TResponse]].handle(request)
  }
}
